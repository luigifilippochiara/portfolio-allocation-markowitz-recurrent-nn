function [fig] = portplot(varargin)
...
for i = 2:nargin
	...
	if numel(plotinfo) > 4
		...
		if isempty(plotstyle)
            % this was b, just changed it to keep colors set by default
            % color palette
			plotstyle = '';
		end
	else
	...
	if strcmpi(plottype,'line')
		for k = 1:size(plotrsk,2)
            % to apply custom rgb colors use
		...
		if ~isempty(plotlabel)
			for k = 1:numel(plotrsk)
				if ~isempty(plotlabel{k})
					text(plotrsk(k) + 0.05, plotret(k), plotlabel{k}, 'FontSize', 12);
				end
			end
		end
	end
...
if ~isempty(plotlegend)
	legend(plotlegend,'Location','NorthEast');
end
